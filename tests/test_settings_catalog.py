...
OUTSIDE = {
    ...
    *{key for key, *_ in websearch.SEARXNG_FIELDS},
    ...
    # Сколько страниц листать у знакомых площадок (reviewsites.PAGED): цена
    # одной — пауза и место в потолке REVIEW_FETCH_PAGES, и подбирается это
    # там же, где остальные пороги загрузки.
    "REVIEW_SITE_PAGES",
    ...
    # жизни установки и вместе с вопросом о лицензии тайлов.
    "MAP_TILE_URL",
    # Потолок разового добора адресов: это значение по умолчанию для
    # geo_backfill.py --limit, а не настройка пайплайна. Кнопка «Адреса для
    # карты» гоняет задачу с --all.
    "GEO_BACKFILL_LIMIT",
