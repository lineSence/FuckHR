...
# ———— калибровка на живых случаях (Сбер, сентябрь 2026) ————
#
# Все четыре проверки ниже — воспроизведение разбора реальной выдачи, где
# «похоже на заказной» получили почти все отзывы компании, включая гневные.


def test_карточка_отзыва_не_делает_отзывы_копиями() -> None:
    """Город, статус и метки площадки — не слова автора.

    Пока они лежали в тексте, два коротких разных отзыва совпадали шинглами
    на 0.9 и получали «почти дословно повторяет другой отзыв».
    """
    meta = "Москва · Бывший сотрудник · белая зарплата · рыночное соответствие рынку"
    items = [
        отзыв("Нет", index=0, rating=1.0, meta=meta),
        отзыв("Не увидела", index=1, rating=1.3, meta=meta),
        отзыв("Не били палкой, ДМС", index=2, rating=3.1, meta=meta),
    ]
    for verdict in fake_reviews.score_items(items):
        assert "dup_same_company" not in verdict.signals


def test_похвала_в_плюсах_при_низкой_оценке_не_рассинхрон() -> None:
    """«Не били палкой, ДМС» с оценкой 1.0 — формат площадки, а не накрутка."""
    item = отзыв(
        "",
        index=0,
        rating=1.0,
        pros="Хороший дружный коллектив, ДМС",
        cons="Руководитель хамит, переработки не оплачивают",
    )
    verdict = fake_reviews.score_items([item])[0]
    assert "rating_mismatch" not in verdict.signals


def test_перечисление_льгот_не_рекламный_хвост() -> None:
    """«Скидки для сотрудников» и «реакция» ловились обрубками «скидк» и «акци»."""
    item = отзыв(
        "ДМС, скидки для сотрудников на ипотеку, быстрая реакция руководства "
        "на просьбы, оклад платят 5 числа",
        index=0,
        rating=4.0,
    )
    verdict = fake_reviews.score_items([item])[0]
    assert "ad_tail" not in verdict.signals

    # Настоящий хвост по-прежнему виден.
    ad = отзыв("Отличная компания. Присоединяйтесь, наши вакансии на www.example.ru", index=1)
    assert "ad_tail" in fake_reviews.score_items([ad])[0].signals


def test_корпусные_сигналы_без_улик_о_тексте_не_дают_заказного() -> None:
    """Десять одинаковых по длине гневных отзывов в одну неделю.

    Раньше «ровная длина» + «всплеск» + «похож на соседа» давали ровно 6
    баллов и метку «похоже на заказной» на живом негативе.
    """
    day = date(2026, 10, 10)
    items = [
        отзыв(
            "Руководитель отделения {} хамит кандидатам, заставил ждать полчаса, "
            "на собеседовании разговаривал свысока и обещал оффер, которого не "
            "было. Оклад ниже рынка, переработки не оплачивают.".format(number),
            index=number,
            rating=1.0,
            dated_at=str(day + timedelta(days=number % 5)),
            date_precision="exact",
        )
        for number in range(10)
    ]
    labels = {v.label for v in fake_reviews.score_items(items)}
    assert fake_rules.LABEL_FAKE not in labels


def test_сгенерированная_хвалебная_пачка_остаётся_заказной(monkeypatch) -> None:
    """Проверка с другой стороны: настоящая накрутка должна ловиться."""
    monkeypatch.setattr(
        fake_reviews.aitext,
        "assess",
        lambda *a, **kw: type("V", (), {"flagged": True})(),
    )
    items = [
        отзыв(
            "Динамично развивающаяся компания, дружный коллектив, современный "
            "офис, перспективы роста. Рекомендую всем! Заботится о сотрудниках.",
            index=number,
            rating=5.0,
            cons="нет",
        )
    ...
    verdicts = fake_reviews.score_items(items)
    assert all(v.label == fake_rules.LABEL_FAKE for v in verdicts)
