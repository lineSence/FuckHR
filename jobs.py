...
TASKS: dict[str, tuple[str, tuple[str, ...], str]] = {
    ...
    # Разметка берётся из своей базы, аргументов из браузера нет: кнопка живёт
    # в разделе «Гейт отзывов», где видно, сколько меток накопилось.
    "gate-train": (
        "Обучение гейта отзывов",
        ("review_gate_train.py",),
        "Учит линейный гейт этапов review_fake и ai_text на накопленной разметке и векторах bge-m3, мерит на отложенной части и пишет веса в базу. Генерации нет; эмбеддер зовётся только на тексты без вектора. Запускается кнопкой в разделе «Гейт отзывов».",
    ),
    # Метки берёт из своей базы, аргументов из браузера нет [CORE-023].
    "stage-gate-train": (
        "Обучение гейта пустоты",
        ("stage_gate_train.py",),
        "Учит гейт пустоты этапа hr_filter на результатах прошлых прогонов и векторах вакансий, мерит на отложенной части по работодателям и пишет веса в базу. Ни генерации, ни сети: эмбеддер не зовётся, векторы берутся готовые.",
    ),
    "tests": (
        "Тесты",
        ("-m", "pytest", "-q"),
