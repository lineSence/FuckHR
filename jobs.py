...
TASKS: dict[str, tuple[str, tuple[str, ...], str]] = {
    ...
    "outreach": (
        "Подготовка писем",
        ("outreach.py",),
        "Собирает черновики по контактам, найденным при сборе, и пишет их в лог. Отправляешь письма всё равно ты сам.",
    ),
    "outreach-dry": (
        "Письма без записи",
        ("outreach.py", "--dry-run"),
    ...
    "target-scan": (
        "Шаг по цели",
        ("target_scan.py",),
        "Собирает вакансии выбранной компании или её отзывы и оценку. Запускается кнопкой в разделе «Цели».",
    ),
    # Старые записи собирались до карты и координат не имеют. Задача ходит по ним
    # порциями и читает адрес со страницы вакансии — внешнего геокодера нет.
    "geo-backfill": (
        "Адреса для карты",
        ("geo_backfill.py",),
        "Добирает адреса и координаты у ранее собранных вакансий, чтобы они появились на карте. Ходит на hh.ru по одной странице с паузами, потолок — GEO_BACKFILL_LIMIT.",
    ),
    "rebuild": (
        "Пересчёт базы",
        ("rebuild.py",),
...
FORM_TASKS = frozenset({"bench", "research-deep"})
