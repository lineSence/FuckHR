...
TASKS: dict[str, tuple[str, tuple[str, ...], str]] = {
    ...
    # в разделе «Laya», где видно, сколько меток накопилось.
    "laya-dataset": (
        "Датасет для Laya",
        ("laya_dataset.py",),
        "Собирает из накопленной разметки датасет для дообучения решателя: оба порядка вариантов, отложенная часть по компаниям. Сеть и модель не трогаются. Запускается кнопкой в разделе «Laya».",
    ),
    "laya-bench": (
        "Laya против текущей модели",
        ("laya_bench.py",),
        "Сравнивает решатель Laya с текущей моделью на этапах review_fake и ai_text по кейсам бенчмарка. Требует pip install laya; при первом запуске качает веса.",
    ),
    "tests": (
        "Тесты",
        ("-m", "pytest", "-q"),
