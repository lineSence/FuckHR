...
Отдельный файл, потому что математика одна и та же для всех гейтов, а данные у
каждого свои: у отзывов это разметка учителя (`review_gate_train.py`), у этапов
вакансии — результат самого пайплайна (`stage_gate_train.py`) [CORE-024].
