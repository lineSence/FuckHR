...
HH_USER_AGENT=FuckHR/0.1 (твоя@почта)
```

`HH_USER_AGENT` обязан содержать почту: без контакта `hh.py` сознательно падает на старте, чтобы ты не улетел в бан анонимных клиентов.
...
|---|---|
| `ValueError: HH_USER_AGENT должен содержать контакт` | в `.env` нет почты в `HH_USER_AGENT` |
