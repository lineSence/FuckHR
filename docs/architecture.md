...
| `hh_html.py`, `hh_parse.py`, `hh.py` | клиент и темп hh.ru, разбор страниц, дампы сбоев в `data/failures` |
...
| `gate_report.py` | цена ворот на карточки при разных дельтах предфильтра |
...
| `laya_dataset.py`, `judge_labels.py`, `training/laya_finetune.py` | разметка учителя рядом с полным текстом, датасет в формате typed-decisions и дообучение Laya (`docs/laya-finetune.md`) |
...
- Интерфейс слушает только `127.0.0.1` и отвечает только своим страницам: `Host` сверяется у каждого запроса, `Origin`/`Referer` и `Sec-Fetch-Site` — у POST (`ui_guard.py`, 23.09.2026). Авторизации нет: наружу его выставлять нельзя.
- `.env` пишется построчно: `settings.save` отказывает значению с переносом строки, `parse_env` снимает экранирование `quote()`.
...
- Темп к каждому домену держит общий на процесс бакет (`net_rate`): поток ждёт
  свою очередь **до** запроса, поэтому паузы перекрываются с латентностью, а не
  складываются с ней. Для hh.ru граница адаптивная: `HH_PAUSE` — верх и точка
  возврата после капчи, `HH_PAUSE_MIN` — низ, до которого интервал сползает на
  чистых ответах; 403/429 и капча сужают бакет сразу для всех потоков.
- Карточки вакансий качает пул (`run_details`, `HH_DETAIL_WORKERS`) под тем же
  бакетом: средний темп к hh.ru не растёт, растёт только заполненность паузы.
...
- Внешний поиск (`websearch.search_many`, `SEARCH_WORKERS`) и страницы отзывов
  ...
  считаться один раз.
- Площадки вакансий обходятся потоками (`sources.py`), hh.ru идёт своим потоком
  рядом с ними: паузы разных доменов не складываются.
- Досье и этапы модели считаются в фоне по мере сбора (`run_bg.Stages`,
  `run_bg.Research`), пока главный поток стоит в паузе перед hh.ru. В общие
  таблицы пишет только главный поток — соединение sqlite между потоками не
  делится.
- Кэш страниц выдачи общий на процесс и протухает по времени
  (`HH_SEARCH_CACHE_MINUTES`), кэш внешнего поиска — по дням
  (`SEARCH_CACHE_DAYS`). Пересборка идёт мимо кэшей сквозным `force`.
- Где прогон всё ещё теряет время и что с этим делать — `docs/performance.md`.
...
- Контакты ищутся один раз на компанию, а не на вакансию: страницы «Команда» и
  ...
  Сами компании расходятся по потокам (`outreach_scan`, `CONTACT_WORKERS`): поток
  открывает своё соединение и провайдер, находки в базу пишет главный поток.
- Одинаковые запросы разных профилей склеиваются в один обход
  (`query_plan`, `collector.collect_plan`): предфильтр и лимит считаются для
  каждого профиля-владельца отдельно.
