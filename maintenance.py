...
TARGETS: tuple[tuple[str, str, tuple[str, ...], str, bool], ...] = (
    (
        ...
        "Кэш внешнего поиска",
        ("search_cache",),
        "Следующий поиск пойдёт в интернет заново. Полезно, если менял движки "
        "или подозреваешь устаревшую выдачу.",
        False,
    ),
    (
        "llm_cache",
    ...
    (
        ...
        ("review_items", "review_hashes", "company_reviews", "company_dossier"),
        "Собирается заново при следующем сканировании, но потратит запросы поиска. "
        "Вместе с досье уходят хэши отзывов: пока они не накопятся заново, "
        "совпадения текстов между компаниями не находятся.",
        False,
    ),
    (
        "contacts",
    ...
    (
        ...
        ("vacancy_conditions", "hr_signals", "vacancies"),
