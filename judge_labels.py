...
Зачем. На этих двух этапах модель отвечает «да/нет» по тексту, и её ответы —
единственная разметка, на которой можно обучить дешёвый гейт
(`review_gate_train.py`). Учить было не на чем: в `review_items` лежит выдержка
в 240 символов, а отрицательный ответ модели не сохранялся вовсе. Здесь лежит
ровно тот текст, что ушёл в промпт, и ответ «да/нет» (решение владельца
23.09.2026, B-22 вопрос 39).
...
владельца, она весит больше учителя и целиком уходит в отложенную часть.
