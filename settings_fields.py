...
FIELDS: tuple[Field, ...] = (
    ...
    Field(
        ...
        BOOL,
        "0",
        "Пишет подробный след прогона в data/diag: какие запросы ушли на "
        "площадки, что вернулось, из чего сложился балл каждой вакансии, что "
        "ответила модель. Файл можно целиком отдать на разбор: секреты и "
        "контакты в него не попадают. Включать на один-два прогона, когда "
    ...
    *(
    ...
    Field(
        ...
        INT,
        "6",
        "Досье собираются потоками: сеть ждёт дольше, чем считает процессор. "
        "Темп обращений держит ограничитель (SEARCH_PAUSE), поэтому потоки не "
        "приближают бан. Максимум 8.",
    ),
    Field(
